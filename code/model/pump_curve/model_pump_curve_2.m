...
%         A'*h.*(edge_type==1) - (diag(L1)*q(1:num_e).^2).*(edge_type==1) + dh_max.*(edge_type==1) <= 0
    ...
    energy_pumps = [energy_pumps; q(pump_edge_list)'*dh_max(pump_edge_list) - L1(pump_edge_list)*pow_p(q(pump_edge_list),3)];
    energy_losses = [energy_losses; L1*pow_p(q(1:num_e),3) - L1(pump_edge_list)*pow_p(q(pump_edge_list),3)];
    gap = [gap; norm(A'*h - diag(L1)*q(1:num_e).^2 + dh_max)];
    gap_pumps = [gap_pumps; norm(A'*h.*(edge_type==1) - (diag(L1)*q(1:num_e).^2).*(edge_type==1) + dh_max.*(edge_type==1))];
    gap_pipes = [gap_pipes; norm(A'*h.*(edge_type~=1) - (diag(L1)*q(1:num_e).^2).*(edge_type~=1))];
    normH = [normH; norm(h)];
...
plot(gap_pumps, 'r')
plot(gap_pipes, 'b')
legend('gap','gap pumps','gap pipes')
title('Gap')

figure;
hold on;
plotyy(1:size(energy_pumps,1),energy_pumps, 1:size(energy_pumps,1),energy_losses)
legend('energy pumps','energy losses')
title('Energy')